...
import { anonymizedActorKey, createFriendsSession, csrfCookie, secretMatches, sessionCookie, type FriendsRole } from '@/lib/friends-auth';
...
export async function POST(request: NextRequest) {
  ...
  if (!env.FRIENDS_SESSION_SECRET || !env.FRIENDS_ACCESS_CODE || !env.OWNER_ACCESS_CODE) {
  ...
  const expected = role === 'owner' ? env.OWNER_ACCESS_CODE : env.FRIENDS_ACCESS_CODE;
  if (!code || !(await secretMatches(code, expected, env.FRIENDS_SESSION_SECRET))) {
  ...
  const created = await createFriendsSession(role, env.FRIENDS_SESSION_SECRET, now);
  const createdAt = new Date(now).toISOString();
  const expiresAt = new Date(created.payload.exp).toISOString();
  await env.FRIENDS_DB.prepare('INSERT INTO friends_sessions (session_id, role, created_at, expires_at) VALUES (?, ?, ?, ?)')
    .bind(created.payload.sid, role, createdAt, expiresAt).run();
